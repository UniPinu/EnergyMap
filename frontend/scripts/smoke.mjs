...
// LIAM semantics: related corridors take the accent stroke (#1ded83); the rest keep the grey default
const lit = await page.$$eval('.react-flow__edge-path', (ps) => ps.filter((p) => getComputedStyle(p).stroke === 'rgb(29, 237, 131)').length)
const grey = await page.$$eval('.react-flow__edge-path', (ps) => ps.filter((p) => getComputedStyle(p).stroke === 'rgb(95, 99, 102)').length)
check(lit === 6 && grey === 37 - 6, `DK1's 6 corridors highlighted in accent green, the other 31 grey (lit=${lit}, grey=${grey})`)
const particles = await page.locator('.react-flow__edges ellipse').count()
check(particles === 6 * 6, `highlighted corridors carry LIAM's 6 travelling particles each (${particles})`)
...
await page.waitForSelector('aside h1', { timeout: 5000 })
const title = await page.locator('aside h1').textContent()
